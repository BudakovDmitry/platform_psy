  ...
  if(!customers.length) {
    return (
        <div className='w-full px-10 py-6'>
          <Card
              isBlurred
              className='border-none my-10 w-full'
              shadow='sm'
          >
            <CardBody>
              <h2 className='text-center font-bold'>Клієнтів поки немає, не засмучуйся!</h2>
            </CardBody>
          </Card>
        </div>
    )
              ...
              return (
                  <tr key={customer._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                          <span
                              onClick={() => sendUpdateCustomer(customer)}
                              className={`inline-flex items-center gap-1 rounded-full ${customer.isActive ? 'bg-green-50 text-green-600' : 'bg-gray-200 text-gray-600'} px-2 py-1 text-xs font-semibold cursor-pointer`}
                          >
                            <span className={`h-1.5 w-1.5 rounded-full ${customer.isActive ? 'bg-green-600' : 'bg-gray-600'}`}></span>
                                Active
                          </span>
                      </td>
                      <td className="px-6 py-4">
